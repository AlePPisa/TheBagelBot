...
from GoogleSearch import get_google_img
from RedditSearch import get_reddit_img
...
async def reddit(message):
    channel = message.channel
    await channel.send(get_reddit_img(message.content))


async def search_google(message):
    channel = message.channel
    await channel.send(get_google_img('nudes' + message.content))
